...
def _normalize_gemini_schema(value: Any) -> Any:
    """Normalize JSON Schema nodes accepted by Gemini function declarations.

    OpenAI-compatible tool schemas may express optional values as a nullable
    union (for example ``["string", "null"]``) and may include
    ``additionalProperties``.  The Gemini gateway used by this deployment
    translates the schemas to protobuf declarations and rejects both shapes.
    Keep this transformation provider-scoped and non-mutating so ordinary
    OpenAI-compatible endpoints retain their original schemas.
    """
    if isinstance(value, dict):
...
def _normalize_gemini_tools(tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Return Gemini-safe copies of OpenAI-style tool definitions."""
    return cast(list[dict[str, Any]], _normalize_gemini_schema(tools))


def _uses_gemini_tool_schema(
    spec: "ProviderSpec | None",
    api_base: str | None,
    model_name: str,
) -> bool:
    """Detect Gemini-backed OpenAI-compatible requests, including custom proxies."""
    return bool(
        (spec and spec.name == "gemini")
        or "gemini" in (api_base or "").lower()
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        if _uses_gemini_tool_schema(spec, self._effective_base, model_name):
            outbound_tools = kwargs.get("tools")
            if isinstance(outbound_tools, list):
                kwargs["tools"] = _normalize_gemini_tools(
                    cast(list[dict[str, Any]], outbound_tools)
                )
    ...
    def _build_responses_body(
        ...
        if _uses_gemini_tool_schema(self._spec, self._effective_base, model_name):
            outbound_tools = body.get("tools")
            if isinstance(outbound_tools, list):
                body["tools"] = _normalize_gemini_tools(
                    cast(list[dict[str, Any]], outbound_tools)
                )
