...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        # The HTTP router handles WebUI mutations; expose the connection set so
        # it can distinguish admin-issued tokens from ordinary WebUI tokens.
        self._http_router._admin_connections = self._admin_connections
    ...
    async def _dispatch_http(self, connection: ServerConnection, request: WsRequest) -> Any:
        ...
        # responses so the WebSocket server does not silently close the request.
        try:
            return await self._http_router.dispatch(connection, request)
        except Exception:
            self.logger.exception("WebUI HTTP dispatch failed for %s", request.path)
