...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    assert any(cmd.command == "image_edit" for cmd in app.bot.commands)
...
async def test_on_message_attaches_reply_to_media_when_available(monkeypatch, tmp_path) -> None:
    ...
    # The replied-to image is held until the user states the desired action.
    assert handled == []
    assert len(app.bot.sent_messages) == 1
    assert "What would you like me to do" in app.bot.sent_messages[0]["text"]

    await channel._on_message(_make_telegram_update(text="analyze the image"), None)

    assert len(handled) == 1
    assert "what is the image?" in handled[0]["content"]
    assert "User instruction: analyze the image" in handled[0]["content"]
...
async def test_on_message_reply_to_caption_and_media(monkeypatch, tmp_path) -> None:
    ...
    assert handled == []
    assert len(app.bot.sent_messages) == 1
    assert "A cute cat" in app.bot.sent_messages[0]["text"]

    await channel._on_message(_make_telegram_update(text="identify the breed"), None)

    assert len(handled) == 1
    assert "[Reply to: A cute cat]" in handled[0]["content"]
    assert "what breed is this?" in handled[0]["content"]
    assert "User instruction: identify the breed" in handled[0]["content"]
    assert len(handled[0]["media"]) == 1
...
def test_telegram_bus_slash_command_regex_matches_agent_loop_commands() -> None:
    ...
    assert pat.fullmatch("/image edit remove the background")
    assert pat.fullmatch("/image_edit remove the background")
    assert TelegramChannel._normalize_telegram_command(
        "/image_edit remove the background"
    ) == "/image edit remove the background"
...
async def test_on_help_includes_restart_command() -> None:
    ...
    assert "/image edit <instruction>" in help_text
    assert "/image_edit <instruction>" in help_text
