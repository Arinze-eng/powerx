...
def begin_automatic_goal(metadata: MutableMapping[str, Any], objective: str) -> bool:
    ...
    metadata[GOAL_STATE_KEY] = {
        ...
        "ui_summary": "Deliberate Telegram task",
...
def is_automatic_goal(metadata: Mapping[str, Any] | None) -> bool:
    """Return whether the current active goal was created by Telegram auto-mode."""
    goal = parse_goal_state(goal_state_raw(metadata))
    return bool(isinstance(goal, dict) and goal.get(AUTO_GOAL_MARKER) is True)


def complete_automatic_goal(metadata: MutableMapping[str, Any], recap: str = "") -> bool:
    """Close an automatic goal after a naturally completed, verified turn."""
    goal = parse_goal_state(goal_state_raw(metadata))
    if not isinstance(goal, dict) or goal.get("status") != "active" or not is_automatic_goal(metadata):
        return False
