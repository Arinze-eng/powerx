    ...
    def get_history(
        ...
        *,
        ...
        for message in sliced:
            ...
            # Synthesize media breadcrumbs for ordinary replayable turns.
            # Telegram VPS/Novita image turns are OCR handoffs, not vision
            # turns; suppress stale local image paths from older sessions so a
            # follow-up cannot trigger read_file or provider image input.
            replay_media = message.get("media")
            if self.key.startswith("telegram:") and role == "user":
                if isinstance(content, str):
                    content = _strip_local_image_breadcrumbs(content)
                replay_media = []
