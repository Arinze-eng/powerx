...
class TurnDelivery:
    ...
    async def running(self, *, started_at: float) -> None:
        if self.route.publish_lifecycle:
            if self.delivery_message.channel == "telegram":
                await self.bus.publish_outbound(
                    OutboundMessage(
                        channel=self.delivery_message.channel,
                        chat_id=self.delivery_message.chat_id,
                        content=(
                            "Task active. I’m working on it and will send the result when it’s finished."
                        ),
                        metadata=self.delivery_message.metadata,
                    )
                )
            elif self.delivery_message.metadata.get("telegram_deliberate_task") is True:
                await self.bus.publish_outbound(
                    outbound_message_for_event(
                        channel=self.delivery_message.channel,
                        chat_id=self.delivery_message.chat_id,
                        event=ProgressEvent(
                            content=(
                                "I’m planning this task carefully first, then I’ll work through "
                                "the steps and verify the result."
                            )
                        ),
                        metadata=self.delivery_message.metadata,
                    )
