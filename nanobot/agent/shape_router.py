...
_LIBRARY_STEER_MESSAGE = (
    ...
    "\n"
    "DESIGN BAR — a deliverable must look designed, never plain:\n"
    "- PDF: read the `pdf-quality` skill (`skills/pdf-quality/SKILL.md`) and "
    "typeset with **pdflatex** (xelatex when system fonts are needed). Raw or "
    "plain-text PDFs, and handing over a `.tex`/`.html`/`.md` instead of a "
    "compiled `.pdf`, are failures. Add real figures: matplotlib charts, TikZ "
    "or Graphviz diagrams, booktabs tables, captioned and embedded.\n"
    "- Slides/deck: read the `deck-design` skill "
    "(`skills/deck-design/SKILL.md`) and build the `.pptx` with python-pptx "
    "using a defined palette, type scale and layout system — title/cover "
    "slides, accent rules, page numbers, KPI cards, section dividers, real "
    "charts. A default white deck of bullets is a failure. Honour the user's "
    "requested colours, fonts, brand, logo or template if they named any.\n"
    "- Images: when the artefact benefits from a photo or illustration, "
    "DOWNLOAD a real one (Wikimedia Commons / Openverse APIs are keyless; "
    "Unsplash/Pexels with a key) and verify it with `file`/`identify` before "
    "embedding. Never embed an unverified download, and never leave a "
    "placeholder where an image was asked for.\n"
    "- Verify before delivering: compile/convert, count the pages/slides, "
    "render the PDF to PNG (or the deck via libreoffice to PDF→PNG) and LOOK "
