...
class AgentLoop:
    ...
    def __init__(
        ...
        execution_config: ExecutionBackendConfig | None = None,
    ...
    async def _run_agent_loop(
        ...
        *,
        ...
        strip_image_content_before_provider: bool | None = None,
        ...
        telegram_image_request = (
            ...
            and (
                ...
                or (
                    ...
                    and (
                        LLMProvider._contains_image_content(provider_state.payload)
                        or LLMProvider._contains_image_content(provider_state.pending_messages)
                    )
                )
            )
        )
    ...
    def _assemble_outbound(
        ...
        *,
        ...
        if (mt := self.tools.get("message")) and isinstance(mt, MessageTool) and mt._sent_in_turn:
            if (not media) and (not had_injections or stop_reason == "empty_final_response"):
                return None
    ...
    @staticmethod
    def _artifact_media_from_messages(messages: list[dict[str, Any]]) -> list[str]:
        """Return existing local artifacts explicitly produced by download_url."""
        marker = "Downloaded remote artifact to local path:"
        paths: list[str] = []
        for message in messages:
            if message.get("role") != "tool" or message.get("name") != "novita_sandbox":
                continue
            content = message.get("content")
            if not isinstance(content, str) or marker not in content:
                continue
            value = content.split(marker, 1)[1]
            value = value.split(" A temporary public download link", 1)[0].strip().rstrip(".")
            if not value:
                continue
    ...
    async def _stage_vps_telegram_attachments(self, ctx: TurnContext) -> None:
        ...
        # read/run/download cycle.
        if not non_image_paths:
            return
        metadata = dict(msg.metadata or {})
        try:
            staged = await analyzer.stage_telegram_attachments(
                non_image_paths,
                session_key=ctx.session_key,
            )
        except Exception as exc:
            metadata["telegram_vps_attachment_stage_error"] = type(exc).__name__
            ctx.msg = dataclasses.replace(
                msg,
                content=(
                    f"{msg.content}\n\n[VPS attachment staging failed: {type(exc).__name__}. "
                    "The original local attachment paths remain available.\n"
                    "Attachments: " + ", ".join(str(path) for path in non_image_paths) + "]"
                ),
                metadata=metadata,
            )
            logger.warning(
                "VPS Telegram attachment staging failed for session {}: {}",
                ctx.session_key,
                type(exc).__name__,
            )
            return
        if not staged:
            return
        lines = ["[Confirmed Telegram attachments staged in the Linux VPS workspace]"]
        for local_path, remote_path in staged:
            lines.append(f"Local: {local_path}\nVPS: {remote_path}")
        metadata["telegram_vps_attachments_staged"] = [
            {"local": local_path, "remote": remote_path}
            for local_path, remote_path in staged
        ]
        metadata["telegram_images_execution_backend"] = "vps"
        ctx.msg = dataclasses.replace(
            msg,
        ...
        logger.info(
            "Staged {} confirmed Telegram attachment(s) in VPS for session {}",
            len(staged),
            ctx.session_key,
        )
    ...
    async def _build_turn(self, ctx: TurnContext) -> None:
        ...
        if ctx.kind is TurnKind.USER:
            if (
                ctx.delivery.route.channel == "telegram"
                and is_deliberate_turn(
                    session.metadata,
                    message_metadata=ctx.msg.metadata,
                )
            ):
    ...
    async def _persist_turn(self, ctx: TurnContext) -> None:
        ...
        if (
            ctx.kind is TurnKind.USER
            and ctx.stop_reason == "completed"
            and not ctx.suppress_response
        ):
            # Automatic goals are a bounded execution aid for one complex user
            # request, not a permanent replacement for the user's session goal.
            # Explicit /goal objectives remain under update_goal control.
            complete_automatic_goal(
                session.metadata,
                recap=(ctx.final_content or "").strip()[:8_000],
            )
        self._save_turn(
    ...
    async def _prepare_outbound(self, ctx: TurnContext) -> None:
        ...
        auto_media: list[str] = []
        if (
            ctx.delivery.delivery_message.channel == "telegram"
            and ctx.stop_reason == "completed"
        ):
            message_tool = self.tools.get("message") if self.tools is not None else None
            if not isinstance(message_tool, MessageTool) or not message_tool.media_sent_in_turn:
                auto_media = self._artifact_media_from_messages(ctx.all_messages)
    ...
    @staticmethod
    def _strip_telegram_image_blocks(messages: list[dict[str, Any]]) -> None:
        """Fail closed: OCR-only Telegram turns must never send image blocks to a model."""
        replacement = (
            "[Telegram image was inspected by the selected sandbox/VPS OCR tool; "
            "raw image content was not sent to the model.]"
        )
        for message in messages:
            content = message.get("content")
