...
def reference_non_image_attachments(
    ...
    """Reference attachments without reading file content into the prompt.

    Image paths are preserved for downstream vision-block construction. Every
    attachment path is also written into the text prompt so the model can use
    ``read_file`` locally or upload the exact source path to ``novita_sandbox``
    ...
    if attachment_refs:
        ...
        guidance = (
            "[Attachment guidance: The paths above are real user-uploaded files. "
            "Use read_file on the exact local path to inspect a file. If a confirmed "
            "VPS path is listed above, use that remote path directly. Otherwise, if "
            "the task must run in Novita Sandbox or VPS, first call novita_sandbox "
            "with action='upload', source set to the exact local path, and a safe "
            "destination under the configured workspace.]"
        )
        suffix = f"{suffix}\n{guidance}"
