...
def _vps_hostkey_response(payload: dict[str, Any] | None = None) -> Response:
    """Discover the live SSH host key fingerprint(s) for a VPS without authenticating.

    Lets an admin see the real ``SHA256:...`` host key the server presents and
    whether it matches the configured ``host_key_fingerprint`` before trusting
    strict mode. Only the host/port are used; no username, password, or private
    key is ever sent by this endpoint.
    """
    try:
        config = apply_render_execution_env(load_config(_config_path()))
        _, vps = _execution_test_config(payload or {}, config)
        if not str(vps.host or "").strip():
            return http_error(400, "Enter a VPS host before scanning its host key")
        keys = discover_vps_host_keys(vps.host, vps.port)
        configured = str(vps.host_key_fingerprint or "").strip()
        fingerprints = [entry["fingerprint"] for entry in keys]
        ...
        return http_error(502, f"Host key scan failed: {str(exc)}")
    except Exception as exc:
        detail = " ".join(str(exc).split())[:320] or type(exc).__name__
        return http_error(502, f"Host key scan failed: {detail}")
...
def admin_route(
    ...
    *,
    ...
    if path == "/api/admin/vps-hostkey":
        return _vps_hostkey_response(payload)
