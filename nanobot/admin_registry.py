...
def _execution_settings() -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "vercel": {
                "api_url": str(getattr(vercel, "api_url", "") or ""),
                "team_id": str(getattr(vercel, "team_id", "") or ""),
                "project_id": str(getattr(vercel, "project_id", "") or ""),
                "runtime": str(getattr(vercel, "runtime", "") or "node22"),
                "vcpus": int(getattr(vercel, "vcpus", 2) or 2),
                "timeout_ms": int(getattr(vercel, "timeout_ms", 1_800_000) or 1_800_000),
                "fetch_allow_hosts": str(getattr(vercel, "fetch_allow_hosts", "") or ""),
                "tokenConfigured": bool(str(getattr(vercel, "token", "") or "").strip()),
                "persistWorkspace": bool(getattr(vercel, "persist_workspace", False)),
            },
            "vps": {
                "host": vps.host,
                "port": vps.port,
...
def _save_execution_settings(
    ...
    *,
    ...
    try:
        ...
        if backend_name not in {"novita", "vps", "upstash", "daytona", "runloop", "vercel"}:
            raise ValueError("execution backend must be novita, vps, upstash, daytona, runloop, or vercel")
        config_path = _config_path()
        ...
        # Vercel Sandbox settings (token is only replaced when a new value is sent).
        vercel = getattr(config.execution, "vercel", None)
        if vercel is not None:
            from nanobot.agent.tools.vercel_backend import (
                validate_vercel_api_url,
                validate_vercel_fetch_allow_hosts,
                validate_vercel_project_id,
                validate_vercel_runtime,
                validate_vercel_team_id,
                validate_vercel_timeout_ms,
                validate_vercel_token,
                validate_vercel_vcpus,
            )

            raw_token = _text(payload, "vercelToken", maximum=512)
            if raw_token:
                vercel.token = validate_vercel_token(raw_token)
            raw_api_url = _text(payload, "vercelApiUrl", maximum=253)
            if raw_api_url:
                vercel.api_url = validate_vercel_api_url(raw_api_url)
            raw_team = _text(payload, "vercelTeamId", maximum=128)
            if raw_team:
                vercel.team_id = validate_vercel_team_id(raw_team)
            raw_project = _text(payload, "vercelProjectId", maximum=128)
            if raw_project:
                vercel.project_id = validate_vercel_project_id(raw_project)
            raw_runtime = _text(payload, "vercelRuntime", maximum=20)
            if raw_runtime:
                vercel.runtime = validate_vercel_runtime(raw_runtime)
            raw_vcpus = payload.get("vercelVcpus")
            if isinstance(raw_vcpus, (int, float)):
                vercel.vcpus = validate_vercel_vcpus(int(raw_vcpus))
            raw_timeout = payload.get("vercelTimeoutMs")
            if isinstance(raw_timeout, (int, float)):
                vercel.timeout_ms = validate_vercel_timeout_ms(int(raw_timeout))
            raw_fetch_hosts = _text(payload, "vercelFetchAllowHosts", maximum=2048)
            if raw_fetch_hosts:
                vercel.fetch_allow_hosts = validate_vercel_fetch_allow_hosts(raw_fetch_hosts)
            raw_persist = payload.get("vercelPersistWorkspace")
            if isinstance(raw_persist, bool):
                vercel.persist_workspace = raw_persist
...
def _test_execution_response(payload: dict[str, Any] | None = None) -> Response:
    try:
        ...
        if backend_name == "vercel":
            from nanobot.agent.tools.vercel_backend import VercelExecutionBackend

            vercel = config.execution.vercel.model_copy(deep=True)
            raw_token = _text(payload or {}, "vercelToken", maximum=512)
            if raw_token:
                vercel.token = raw_token
            raw_api_url = _text(payload or {}, "vercelApiUrl", maximum=253)
            if raw_api_url:
                vercel.api_url = raw_api_url
            raw_team = _text(payload or {}, "vercelTeamId", maximum=128)
            if raw_team:
                vercel.team_id = raw_team
            raw_project = _text(payload or {}, "vercelProjectId", maximum=128)
            if raw_project:
                vercel.project_id = raw_project
            if not str(vercel.token or "").strip():
                return http_error(400, "Vercel token is required to test the Vercel Sandbox backend")
            tested = asyncio.run(
                VercelExecutionBackend(vercel, sandbox_name="powerx-connection-test").test_connection()
            )
            return http_json_response({"ok": bool(tested.get("ok")), "backend": "vercel", **tested})
        if backend_name != "vps":
