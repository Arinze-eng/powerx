...
| `engineering-draw` | Real 2D engineering drawings and 3D CAD parts, inside the execution sandbox: editable DXF `DIMENSION` entities on layers with a title block (`ezdxf`), and solids exported as STEP/STL/3MF plus projected, sectioned and hatched views (`build123d`) — backing the `engineering_draw` tool |
