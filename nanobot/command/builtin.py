...
BUILTIN_COMMAND_SPECS: tuple[BuiltinCommandSpec, ...] = (
    ...
    BuiltinCommandSpec(
        "/uniabuja",
        "Query UniAbuja transcript",
        "Run a safe UniAbuja transcript query; access depends on administrator policy switches.",
        "database",
        "<student|score|payment|eligibility|schema> ...",
        accepts_args=True,
    ),
)
...
async def cmd_uniabuja(ctx: CommandContext) -> OutboundMessage:
    """Run a deterministic, policy-gated UniAbuja transcript operation."""
    metadata = dict(ctx.msg.metadata or {})
    try:
        tokens = shlex.split(ctx.args.strip())
    except ValueError:
        tokens = []
    if not tokens:
        usage = (
            "Usage:\n"
            "/uniabuja student <regno> [session]\n"
            "/uniabuja score <regno> <course> <session> <semester>\n"
            "/uniabuja payment <regno> <session> <semester>\n"
            "/uniabuja eligibility <regno> <session> <semester>\n"
            "/uniabuja schema <table>\n\n"
            "Students are automatically restricted to their own authenticated portal record."
        )
        return OutboundMessage(
            channel=ctx.msg.channel,
            chat_id=ctx.msg.chat_id,
    ...
    tool = ctx.loop.tools.get("uniabuja_transcript")
    if tool is None:
        return OutboundMessage(
            channel=ctx.msg.channel,
            chat_id=ctx.msg.chat_id,
            content="UniAbuja transcript gateway is not configured on the server.",
            metadata={**metadata, "render_as": "text"},
        )

    command = tokens[0].lower()
    params: dict[str, Any]
    if command == "student" and len(tokens) in {2, 3}:
        params = {"action": "student_lookup", "regno": tokens[1]}
        if len(tokens) == 3:
            params["session"] = tokens[2]
    elif command == "score" and len(tokens) == 5:
        params = {
            "action": "score_check",
            "regno": tokens[1],
            "course": tokens[2],
    ...
    elif command == "payment" and len(tokens) == 4:
        params = {
    ...
    elif command == "eligibility" and len(tokens) == 4:
        params = {
    ...
    elif command == "schema" and len(tokens) == 2:
        params = {"action": "schema", "table": tokens[1]}
    else:
        return OutboundMessage(
            channel=ctx.msg.channel,
            chat_id=ctx.msg.chat_id,
            content="Invalid /uniabuja syntax. Use /uniabuja without arguments for usage.",
            metadata={**metadata, "render_as": "text"},
        )
    ...
    request_token = bind_request_context(RequestContext(
        ...
        session_key=ctx.key,
        original_user_text=ctx.msg.content,
        runtime=ctx.runtime,
        metadata=metadata,
        sender_id=ctx.msg.sender_id,
        turn_id=metadata.get("webui_turn_id"),
        workspace=str(ctx.loop.workspace),
    ))
    try:
...
def register_builtin_commands(router: CommandRouter) -> None:
    ...
    router.exact("/uniabuja", cmd_uniabuja)
    router.prefix("/uniabuja ", cmd_uniabuja)
