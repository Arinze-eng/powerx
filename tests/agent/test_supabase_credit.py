...
@pytest.mark.asyncio
async def test_credit_hook_charges_every_step_before_it_runs(monkeypatch) -> None:
    """One RPC per step, numbered from 1, charged ahead of the iteration."""
    fake = FakeSupabase()
...
@pytest.mark.asyncio
async def test_credit_hook_stops_the_task_the_moment_credit_runs_out(monkeypatch) -> None:
    """A refused step raises immediately, so nothing further is attempted."""
    fake = FakeSupabase()
    fake.failure = SupabaseAuthError("Insufficient credits: this step costs 15 and the balance is 0")
    monkeypatch.setattr(supabase_credit, "SupabaseAuth", lambda: fake)
    hook = supabase_credit.SupabaseCreditHook(AgentTurnHookContext(
        channel="telegram",
        chat_id="42",
    ...
    with pytest.raises(supabase_credit.CreditExhaustedError, match="Add credit"):
        await hook.before_iteration(AgentHookContext(iteration=0, messages=[]))
    # Nothing was billed and no step counted, so a finished balance cannot
    # start a task either.
    assert hook.charged_steps == 0
