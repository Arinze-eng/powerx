...
def test_discover_vps_host_keys_parses_ssh_keyscan_output(monkeypatch) -> None:
    from nanobot.agent.tools import vps_backend

    fake_output = (
        "3072 SHA256:rsaFPxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx vps.example.test (RSA)\n"
        "256 SHA256:ecdsaFPyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy vps.example.test (ECDSA)\n"
        "256 SHA256:edfpzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz vps.example.test (ED25519)\n"
    ...
    monkeypatch.setattr(vps_backend.shutil, "which", lambda name: "/usr/bin/" + name)
    monkeypatch.setattr(vps_backend.subprocess, "run", lambda *a, **k: FakeCompleted())

    keys = vps_backend.discover_vps_host_keys("vps.example.test", 22)
    types = {k["key_type"] for k in keys}
    assert types == {"rsa", "ecdsa", "ed25519"}
    fps = [k["fingerprint"] for k in keys]
    assert all(fp.startswith("SHA256:") for fp in fps)
    assert all(k["bits"].isdigit() for k in keys)


def test_discover_vps_host_keys_rejects_bad_port_and_missing_tools(monkeypatch) -> None:
    from nanobot.agent.tools import vps_backend

    monkeypatch.setattr(vps_backend.shutil, "which", lambda name: None)
    import pytest as _pytest
    with _pytest.raises(RuntimeError):
        vps_backend.discover_vps_host_keys("vps.example.test", 22)


def test_vps_execution_backend_discover_host_keys_matches_configured(monkeypatch, vps_config) -> None:
    from nanobot.agent.tools import vps_backend

    monkeypatch.setattr(
        vps_backend, "discover_vps_host_keys",
        lambda host, port, **kw: [
            {"key_type": "ed25519", "bits": "256", "fingerprint": "SHA256:testfingerprint"},
            {"key_type": "rsa", "bits": "3072", "fingerprint": "SHA256:OTHER"},
        ],
    )
    result = vps_backend.VPSExecutionBackend(vps_config).discover_host_keys()
    assert result["ok"] is True
    assert result["matches"] is True
    assert result["configured_fingerprint"] == "SHA256:testfingerprint"
