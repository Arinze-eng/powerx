...
async def test_document_attachment_is_referenced_and_read_on_demand(
    ...
    assert f"import this report\n\n[Attachment: {csv_path}]" in ctx.msg.content
    assert "Use read_file on the exact local path" in ctx.msg.content
...
async def test_document_reference_survives_session_reload(tmp_path: Path) -> None:
    ...
    assert "first call novita_sandbox" in persisted.messages[0]["content"]
...
async def test_pending_document_attachment_keeps_body_out_of_prompt(
    ...
    assert "first call novita_sandbox" in injected_user_content
...
def test_attachment_references_still_preserve_images(tmp_path: Path) -> None:
    ...
    assert "first call novita_sandbox" in content
