...
@pytest.mark.asyncio
async def test_prepare_telegram_images_forwards_direct_fetch_token_to_vision(monkeypatch) -> None:
    """Direct-fetch image tokens must reach the vision model, not Tesseract OCR."""
    from nanobot.agent.tools.novita_sandbox import _encode_telegram_url_token

    url = "https://api.telegram.org/file/bot123/photos/photo_9.jpg"
    token = _encode_telegram_url_token(url, "photo_9.jpg")
    analyze = AsyncMock()
    monkeypatch.setattr(NovitaSandboxTool, "analyze_telegram_images", analyze)

    ctx = SimpleNamespace(
        kind=TurnKind.USER,
        session_key="telegram:138",
        msg=InboundMessage(
            channel="telegram",
            sender_id="42",
            chat_id="138",
    ...
    # Tesseract must NOT run for direct-fetch tokens (vision path instead).
    analyze.assert_not_awaited()
    # The token stays in media so build_user_content turns it into a remote image_url.
    assert ctx.msg.media == [token]
    assert ctx.msg.metadata.get("telegram_images_via_vision_url") is True
    assert "forwarded to the vision model" in ctx.msg.content
    assert "never transit the Render host" in ctx.msg.content


def test_build_user_content_telegram_token_produces_remote_image_block() -> None:
    from nanobot.agent.context import ContextBuilder

    builder = ContextBuilder.__new__(ContextBuilder)  # type: ignore[arg-type]
    url = "https://api.telegram.org/file/bot1/photos/img.png"
    token = f"tgurl::{url}::img.png"
    result = builder.build_user_content("describe", [token])
    assert isinstance(result, list)
    image_blocks = [b for b in result if b.get("type") == "image_url"]
    assert len(image_blocks) == 1
    assert image_blocks[0]["image_url"]["url"] == url
    assert image_blocks[0]["_meta"].get("remote") is True
    texts = [b.get("text", "") for b in result if b.get("type") == "text"]
    assert texts == ["describe"]
