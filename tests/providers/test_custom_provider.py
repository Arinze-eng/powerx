...
def test_custom_provider_strips_internal_custom_model_prefix() -> None:
    spec = find_by_name("custom")
    with patch("nanobot.providers.openai_compat_provider.AsyncOpenAI"):
        provider = OpenAICompatProvider(
            api_key="custom-key",
            default_model="custom/muse-spark-1.2-contributor-free",
            spec=spec,
        )
    ...
    kwargs = provider._build_kwargs(
        ...
        tools=None,
        model="custom/muse-spark-1.2-contributor-free",
        max_tokens=128,
        temperature=0.7,
    ...
    assert kwargs["model"] == "muse-spark-1.2-contributor-free"


def test_custom_gemini_reasoning_effort_is_forwarded() -> None:
    spec = find_by_name("custom")
    with patch("nanobot.providers.openai_compat_provider.AsyncOpenAI"):
        provider = OpenAICompatProvider(
            api_key="custom-key",
            default_model="custom/gemini-3.1-flash-lite",
            spec=spec,
        )
    ...
    kwargs = provider._build_kwargs(
        ...
        tools=None,
        model="custom/gemini-3.1-flash-lite",
        max_tokens=256,
        temperature=0.1,
