...
def test_vps_hostkey_scan_endpoint_reports_match(monkeypatch, tmp_path: Path) -> None:
    """The admin /api/admin/vps-hostkey route scans live keys and flags a match."""
    config_path = tmp_path / "config.json"
    source = Path(__file__).parents[1] / "render-config.json"
    config_path.write_text(source.read_text(), encoding="utf-8")
    monkeypatch.setenv("ADMIN_PASSWORD", "nethunter")
    ...
    # Persist a VPS host + a fingerprint equal to one of the discovered keys.
    save_req = _request("/api/admin/execution-settings")
    save_req._nanobot_webui_mutation_payload = {
        "backend": "vps",
        "host": "vps.example.test",
        "port": "22",
        "username": "administrator",
        "password": "test-password",
        "privateKey": "",
        "hostKeyFingerprint": "SHA256:AAAAMATCHEDKEYAAAAAAAAAAAAAAAAAAAAAAAAAAA",
        "hostKeyPolicy": "accept_any",
        "workspaceDir": "/workspace",
        "connectTimeout": "20",
    }
    saved = admin_registry.admin_route(save_req, "/api/admin/execution-settings")
    assert saved is not None and saved.status_code == 200

    discovered = [
        {"key_type": "ed25519", "bits": "256", "fingerprint": "SHA256:AAAAMATCHEDKEYAAAAAAAAAAAAAAAAAAAAAAAAAAA"},
        {"key_type": "rsa", "bits": "3072", "fingerprint": "SHA256:BBBBOTHERKEYBBBBBBBBBBBBBBBBBBBBBBBBBBB"},
    ]
    monkeypatch.setattr(admin_registry, "discover_vps_host_keys", lambda host, port, **kw: discovered)

    req = _request("/api/admin/vps-hostkey")
    req._nanobot_webui_mutation_payload = {}
    response = admin_registry.admin_route(req, "/api/admin/vps-hostkey")
    assert response is not None
    assert response.status_code == 200
    body = json.loads(bytes(response.body).decode())
    assert body["ok"] is True
    assert body["matches"] is True
    assert body["configured_fingerprint"] == "SHA256:AAAAMATCHEDKEYAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    assert len(body["keys"]) == 2


def test_vps_hostkey_scan_endpoint_requires_auth(monkeypatch, tmp_path: Path) -> None:
    config_path = tmp_path / "config.json"
    source = Path(__file__).parents[1] / "render-config.json"
    config_path.write_text(source.read_text(), encoding="utf-8")
    monkeypatch.setenv("ADMIN_PASSWORD", "nethunter")
    monkeypatch.setattr(admin_registry, "_config_path", lambda: config_path)
    unauthorized = admin_registry.admin_route(
        SimpleNamespace(path="/api/admin/vps-hostkey", headers={}),
