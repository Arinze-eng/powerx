...
# list keeps the image configurable while keeping Telegram available by default.
ARG NANOBOT_CHANNELS=telegram
...
RUN sed -i 's/\r$//' /usr/local/bin/entrypoint.sh && chmod +x /usr/local/bin/entrypoint.sh && \
    sed -i 's/\r$//' /app/scripts/nanobot_launcher.sh && chmod +x /app/scripts/nanobot_launcher.sh
